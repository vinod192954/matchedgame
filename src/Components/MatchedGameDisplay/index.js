...
import TabItem from '../Thumbnails'
...
class MatchedGame extends Component {
  state = {
    activeTabId: tabsList[0].tabId,
    score: 0,
    matchedImage: imagesList[0].id,
    isMatched: true,
    elapsedSeconds: 60,
  }

  componentDidMount = () => {
    const {elapsedSeconds, isMatched, score} = this.state
  ...
  getRandomImage = () => {
    ...
    const imageIndex = imagesList.length
    const randomImage = Math.floor(Math.random() * imageIndex)
    const imageObject = imagesList[randomImage]
    return imageObject.id
  }

  onClickedImageDisplay = id => {
    const {matchedImage, isMatched} = this.state
    if (id === matchedImage) {
      this.setState(prevState => ({score: prevState.score + 1}))
      this.setState({matchedImage: this.getRandomImage()})
  ...
  playGameAgain = () => {
    const {isMatched, score} = this.state
    this.setState({isMatched: true})
  ...
  decrementTimer = () => {
    const {elapsedSeconds, isMatched} = this.state
    if (elapsedSeconds > 0) {
  ...
  render() {
    ...
    const displayImage = imagesList.find(image => image.id === matchedImage)
      .imageUrl
        ...
        <div className="header-container">
          ...
          <div className="score-timer-container">
            ...
            <div className="seconds-container">
              ...
              />
              <p className="seconds">{elapsedSeconds}sec</p>
            </div>
          </div>
        </div>
        <div>
          {isMatched ? (
            <div className="content-container">
              <img src={displayImage} alt="match" className="image-display" />
              ...
              <ul className="fruits-items-container">
                {filteredImages.map(eachImage => (
                  <FruitsItems
                    FruitItem={eachImage}
                    key={eachImage.id}
                    onClickedImageDisplay={this.onClickedImageDisplay}
                  />
                ))}
              </ul>
            </div>
          ) : (
            <div className="score-dsplayer">{this.displayResultImage()}</div>
          )}
        </div>
      </div>
    )
